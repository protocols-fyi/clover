...
| Name         | Type                        | Description                                                                                                                           |
| :----------- | :-------------------------- | :------------------------------------------------------------------------------------------------------------------------------------ |
| `input`      | `z.infer<z.ZodObject<any>>` | Input variables as described by your server route input schema.                                                                       |
...
| `validator?` | `z.ZodObject<any>`          | Parse the received response with a Zod schema. It must be the same as the server output schema or you will get a type error.          |
